...
class CreateHallAllotmentRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if ($this->student_id) {
                $student = \App\Models\Student::with('department')->find($this->student_id);
                
                if ($student && $student->department && !$student->department->is_active) {
                    $validator->errors()->add('student_id', 'Cannot create allotment. Student\'s department is inactive.');
                }
            }
        });
    }
